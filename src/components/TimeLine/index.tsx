...
const data = {
  ...
  stageList: [
    ...
    { id: "3", title: "淘特游戏时⻓统计脚本/前端核⼼开发/2022.7～2022.8", desc: '该项⽬专为游戏⼤厅及各个游戏⻓提供⽤户游戏时⻓的统计服务以及游戏⼤厅流量去向的追踪。 参与 了需求评审、现有⽅案调研、技术⽅案设计、代码的编写实现、测试bug修复等⼯作，开发过程中主要 解决了统计数据准确性（采集和处理）、防作弊和安全性、与宿主⻚ ⾯（⾃研游戏和三⽅接⼊游戏） 相隔离、异常处理、配置能⼒、后续复⽤扩展与维护（如⽀持ui渲染）等问题。 上线后保持平稳的运 ⾏，为游戏⼤厅提供稳定的时⻓统计服务。技术栈：preact、js', status: "-1", threshold: "300" },
    { id: '4', title: '智慧园区后台管理系统/前端负责⼈/2018.5～2019.5', desc: '该项⽬为⽅便⾼效的对园区消防、空调、电梯及⻔禁、硬件设备等管理维护和控制以及相关数据采集 并且可⼤屏可视化进⾏展示。 参与系统的需求评审，根据设计稿给出可⾏的技术⽅案进⾏编码实现， 完成了⽤户⻆⾊权限管理、报警管理、数据报表、楼层电梯管理、设备远程控制，⼤屏实时数据展 示，⻚⾯⽂案的国际化处理等功能。前端采取技术栈：vue+vuex+element-ui 沉淀出⼀套基于 element-ui进⾏⼆次封装的适⽤于公司内部适⽤的公⽤业务组件，树表、树形选择器、树形菜单导航、 基础表格、表单、表单表格等。封装过可拖拽的⾃定义指令，并在团队内部使⽤。' }
  ]
}
      ...
      <div className="timeLineListWrap" ref={scrollRef}>
        <div className="timeLineList">
          {stageList.map((item, index) => {
            return (
              <div className="timeLineItemWrap" key={item.id}>
                <Progress
