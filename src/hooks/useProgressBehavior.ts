...
export default function useProgressBehavior(props: IProps) {
  ...
  const isHorizontal = useMemo(() => direction === DirectionEnum.Row, [
    direction
  ]);

  const barStyle: IBarStyle = {
    transform: isHorizontal
      ? `translateX(${barPercent})`
      : `translateY(${barPercent})`,
    transition
  };
  const style: IStyle = {
  ...
  useEffect(() => {
    ...
    target &&
      target.addEventListener("webkitTransitionEnd", handleChange, false);
    return () => {
      target &&
        target.removeEventListener("webkitTransitionEnd", handleChange, false);
    };
  }, [percent]);
